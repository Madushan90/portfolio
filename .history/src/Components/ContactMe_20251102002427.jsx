...
const ContactMe = () => {
    // State to hold the submission result (success/error message)
    const [result, setResult] = useState("");
    // Ref to access the form element directly for clearing
    ...
    const onSubmit = async (event) => {
        ...
        // 🚨 ADD THIS LINE to enable hCaptcha protection
        ...
        try {
            ...
            if (data.success) {
                ...
                // If Web3Forms returns an error (e.g., failed hCaptcha)
            ...
            <div>
                <div className='p-5 bg-cyan-800/10 rounded-2xl'>
                    ...
                    <form onSubmit={onSubmit} ref={formRef}>
                        ...
                        <div>
                            <button type='submit' className='inline-block text-amber-300 border-2 border-double border-amber-500 rounded-3xl px-4 py-2.5 text-center bg-transparent transition-all duration-200 hover:shadow-xl hover:bg-amber-500 hover:text-white w-30'>Submit</button>
                            {/* Display the result message here */}
                            {result && <p className={`mt-3 font-semibold ${result.includes("success") ? 'text-green-400' : 'text-red-400'}`}>{result}</p>}
                        </div>
                    </form>
                </div>
            </div>
