...
const Navbar = () => {
  ...
  return (
      <nav>
        ...
        {/* Mobile Menu */}
        <Menu onClick={() => setsideBarOpen(true)} className='text-white mt-3 w-8 h-8' />
        <div class={`${sideBar ? "translate-x-0" : "translate-x-full"} fixed inset-y-0 right-0 z-50 w-80 bg-white/10 backdrop-blur-2xl border-r border-white/20 transform transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
          <X onClick={() => setsideBarOpen(false)} className='text-white mt-3 w-8 h-8' />
          <AnchorLink href='#home'><p onClick={()=>setMenu("Home")} className="relative cursor-pointer rounded-full px-3 py-2 font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Home</span>{menu==="Home"? <img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          ...
          <AnchorLink offset={-80} href='#services'><p href="services" onClick={()=>setMenu("Services")} className="relative cursor-pointer rounded-full px-3 py-2 font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Services</span>{menu==="Services"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-80} href='#portfolio'><p href="#" onClick={()=>setMenu("Portfolio")} className="relative cursor-pointer rounded-full px-3 py-2 font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Portfolio</span>{menu==="Portfolio"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-80} href='#contact'><p href="#" onClick={()=>setMenu("Contact")} className="relative cursor-pointer rounded-full px-3 py-2 font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Contact</span>{menu==="Contact"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
        </div>
