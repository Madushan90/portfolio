            ...
            <div className="flex items-center justify-between h-26">
              ...
              <div class="hidden sm:ml-6 md:block"> 
                <div class="flex space-x-4">
                  ...
                  <AnchorLink href='#services'><p href="services" onClick={()=>setMenu("Services")} className="cursor-pointer rounded-full px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Services</span>{menu==="Services"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
                  <AnchorLink href='#portfolio'><p href="#" onClick={()=>setMenu("Portfolio")} className="cursor-pointer rounded-full px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Portfolio</span>{menu==="Portfolio"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
                  <AnchorLink href='#contact'><p href="#" onClick={()=>setMenu("Contact")} className="cursor-pointer rounded-full px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white"><span>Contact</span>{menu==="Contact"?<img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
                </div>
              </div>
              <div>
