      ...
      <nav>
        ...
        <div class={`${sideBar ? "translate-x-0" : "translate-x-full"} md:hidden fixed inset-y-0 right-0 z-50 w-80 bg-white/10 backdrop-blur-2xl border-r border-white/20 transform transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
          <div className='flex pr-4 justify-end'>
            <X onClick={() => setsideBarOpen(false)} className='text-white my-3 w-8 h-8 cursor-pointer hover:text-amber-400' />
          </div>
          <AnchorLink href='#home'><p onClick={()=>setMenu("Home")} className="relative cursor-pointer px-5 py-4 font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>Home</span>{menu==="Home"? <img className='absolute top-3 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-100} href='#about'><p href="#" onClick={()=>setMenu("About")} className="cursor-pointer px-3 py-4 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>About Me</span>{menu==="About"?<img className='absolute top-3 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-80} href='#services'><p href="services" onClick={()=>setMenu("Services")} className="cursor-pointer px-3 py-4 font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>Services</span>{menu==="Services"?<img className='absolute top-3 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-80} href='#portfolio'><p href="#" onClick={()=>setMenu("Portfolio")} className="cursor-pointer px-3 py-4 font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>Portfolio</span>{menu==="Portfolio"?<img className='absolute top-3 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
          <AnchorLink offset={-80} href='#contact'><p href="#" onClick={()=>setMenu("Contact")} className="cursor-pointer px-3 py-4 font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>Contact</span>{menu==="Contact"?<img className='absolute top-3 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
        </div>
