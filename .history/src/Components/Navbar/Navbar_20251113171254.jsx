...
const Navbar = () => {
  ...
  return (
      <nav>
        ...
        {/* Mobile Menu */}
        <div className="md:hidden fixed flex justify-end bg-red-500 w-full px-15 py-1">
          <Menu onClick={() => setsideBarOpen(true)} className='text-white w-8 h-8 cursor-pointer hover:text-amber-400' />
        </div>
        <div class={`${sideBar ? "translate-x-0" : "translate-x-full"} md:hidden fixed inset-y-0 right-0 z-50 w-80 bg-white/10 backdrop-blur-2xl border-r border-white/20 transform transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
          <div className='flex justify-end'>
            <X onClick={() => setsideBarOpen(false)} className='text-white pl-2 w-8 h-8 cursor-pointer hover:text-amber-400' />
          </div>
