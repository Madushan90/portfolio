  ...
  return (
      <nav>
        ...
        <div class={`${sideBar ? "translate-x-0" : "translate-x-full"} md:hidden fixed inset-y-0 right-0 z-50 w-80 bg-white/10 backdrop-blur-2xl border-r border-white/20 transform transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
          ...
          <AnchorLink href='#home'><p onClick={()=>setMenu("Home")} className="relative cursor-pointer rounded-full px-3 py-1 font-medium text-gray-300 hover:bg-white/5 hover:text-white mb-3"><span>Home</span>{menu==="Home"? <img className='absolute top-1 left-1 w-17 rotate-8' src={Underline} alt='underline-pattern' /> : <></>}</p></AnchorLink>
